...
\eq{\label{WeakHarnackEv2}\br{\del_t-\Box}\br{tc\operatorname{tr}(\dot{\p})\p}&=tc\Big(\operatorname{tr}(\dot{\p})\p\p^{ij}(h^2)_{ij}+c\operatorname{tr}(\dot{\p})^2\p+\p\del_t\operatorname{tr}(\dot{\p})\\
                    &\hp{-tk\Big(}-\p\Box\operatorname{tr}(\dot{\p})-2\p^{ij}\nabla_{i}\p\nabla_{j}\operatorname{tr}(\dot{\p})\Big)\\
                            &\hp{=}+c\operatorname{tr}(\dot{\p})\p.
                            }
Noting that
...
$$\p''+\fr{2\p'}{f}-\fr{1}{\d}\fr{\p'^2}{\p}\geq 0,\quad \d\p(0,\xi)>0.$$
Then
$\chi_2=t(\partial_t\p-b^{ij}\nabla_i\p\nabla_j\p)+\d\p>0$
 along \eqref{eq:CurvFlow}.
\end{corollary}
...
These results are not entirely optimal. We refer to \cite{AndrewsMcCoyZheng:07/2013} for an exhaustive discussion of the contracting case.
