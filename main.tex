...
Q = \partial_t \p - b^{ij} \nabla_i \p \nabla_j \p
\]
where \(b^{ij}\) is the inverse of the second fundamental form of a strictly convex hypersurface. Several authors, beginning with \cite{} have investigated this quadratic\cite{}, relating it to the second fundamental form of a degenerate metric on space-time.
...
\]
where \(\Box\) is the elliptic operator \(\p^{ij} \nabla^2_{ij}\) and \(\p^{ij}\) is the differential of the speed function (see the next section below) and \((h^2)_{ij}\) is a quadratic metric contraction of the second fundamental form. One now computes the evolution of \(\Box u - \p^{ij}(h^2)_{ij} - \p^{ij}g_{ij} = \partial_t u - \p^{ij} \nabla_i u \nabla_j u\) and seeks to apply the maximum principle (in the form of an ODE comparison) to show that
\[
\partial_t u - \p^{ij} \nabla_i u \nabla_j u \geq -\frac{C}{t},
\]
for an appropriate choice of \(C\). Or in other words,
\[
\frac{\partial_t \p}{\p} + \frac{\p^{ij} \nabla_i \p \nabla_j \p}{\p^2}  + \frac{C}{t} \geq 0,
\]
the Li-Yau differential Harnack inequality.

For geometric hypersurface flows, this does not quite work. We need to replace the \(\p^{ij} \nabla_i u \nabla_j u\) term with \(b^{ij} \nabla_i \p \nabla_j \p\) in the Harnack quadratic. It is not so surprising that the Li-Yau formulation is insufficient for hypersurface flows since the evolution of the speed \(\p\) depends not only on \(\p\) and it's derivatives, but also on the \emph{changing} metric and second fundamental form which are changing in time, hence one needs to introduce additional ``compensating terms'' for the maximum principle to work. To determine the appropriate terms, Hamilton supposed the equality case should hold for self-similar solutions.

In the Gauss parametrisation used in \cite{MR1296393}, the Harnack quadratic is simply \(\partial_t \p\). Transforming back to the curvature flow parametrisation (called the ``standard'' parametrisation in \cite{MR1296393}), miraculously re-introduces \emph{exactly the missing term} and gives the Harnack inequality! This method is quite close in philosophy to the Li-Yau approach, working with the time-independent standard metric on the sphere.
...
For the second question, note that the Gauss map may be considered as a map \(M_t \to \S^{n+1}\), the image of which is a time-dependent convex hypersurface (assuming that \(M_t\) is itself convex) \cite{Gerhardt:/2006}. Thus the Gauss map parametrisation of \(M_t\) is not defined on fixed convex hypersurface, but rather a changing convex hypersurface of \(\S^{n+1}\) and this approach has significantly less utility than in the Euclidean case. There are various other possible candidates, but we found none that worked as well as in the Euclidean case except for certain special cases. It would be quite interesting to determine if there exists a ``canonical'' parametrisation for strictly convex hypersurfaces of the sphere, compatible with the Harnack quadratic and  analogous to the Gauss map parametrisation.

The first question offers more possibilities. The usual symmetries considered in Euclidean space are isometries of the ambient space and space-time scalings. For the Harnack inequality, rotations are not so important, but translations and scalings are of utmost importance. On the sphere, if the initial area is smaller than the area of an equator, under suitable curvature assumptions, \cite[Theorem 0.1]{MR892052} states that the hypersurface collapses to a point and thus cannot move by isometry. Otherwise, it converges smoothly to an equator and hence must have been an equator for all time (since it moves by isometries). Therefore, there are no closed, convex hypersurfaces of the sphere moving by isometry. This is the same in Euclidean space of course, but in that situation one may also consider translators, which are non-compact and so don't exist in the sphere. 

As to scalings, these don't exist on the sphere, but there is a one-parameter subgroup of conformal transformations somewhat analogous to scalings: the flow of the ``spherical position vector field'' \(\posvec = \sin(\radialdistance) \grad \radialdistance\) where \(\radialdistance\) denotes the distance to a fixed point. This is the unique, radially pointing conformal vector field, and in fact, realising the sphere as locally conformally flat via steriographic projection from this point, corresponds to scalings. This one-parameter subgroup is not exactly a symmetry of our flows, but we can garner some intuition from it.

Such, ``conformal solitons'' are characterised by the equation
\[
\ip{\posvec}{\nor} = -c(t) \p
\]
where \(c(t)\) can be any positive function (for shrinking flows). This is equivalent to requiring that a solution of the curvature flow \(\del_t x = - \p \nu\) can re-parametrised so that \(M_t = f_t(F_0(M))\) where \(f_t\) is the flow of \(\posvec\). 

An example of a solution to our curvature flows moving by this family is the shrinking geodesic spheres. Here we are only interested in attaining a qualitative understanding of the connection between solitons and Harnack inequalities, so let us consider the case of the Mean Curvature Flow. In this case, for a family of shrinking geodesic spheres \(\{\radialdistance = \radialdistance(t) = \arccos(e^{-t})\}\) (see e.g. \cite{Gerhardt:/2015} equations 6.5,6.11), the conformal-soliton equation becomes
\[
\ip{\sin(\radialdistance) \partial_{\radialdistance}}{\partial_{\radialdistance}} = - c(t) n \cot (\radialdistance)
\]
where \(\radialdistance\) measures the distance from the point of collapse. Thus 
\[
c(t) = -\frac{1}{n} \frac{\sin^2(\radialdistance(t))}{\cos(\radialdistance(t))} = \frac{2}{n} \sinh(t).
\]
On the other hand, differentiating the conformal soliton equation with respect to \(t\) gives
\[
\cos(\radialdistance) \del_t \radialdistance H = -c' H + c \del_t H
\]
since \(\del_t \nu = \grad H = 0\). Simplifying we obtain the identity
\[
0 = \del_t H + \frac{1}{1 - e^{-2t}} H = \del_t H + (\frac{1}{2t} + 1/2 + (1/6) t + \cdots) H.
\]
Now we observe that the Harnack inequality (\cref{thm:harnack} with \(\p = H\), or \cite{IvakiBryan:08/2015} states that
\[
\del_t H + \left(\frac{1}{2t}  - n\right) H \geq 0
\]
whenever \(\nabla H = 0\).

Thus we find that the Harnack inequality give in \cref{thm:harnack} is not sharp. Note however, that the first order term is \(2t\) and so our Harnack is ``sharp to first order''. The lack of a sharp Harnack is perhaps not too surprising given that conformal-solitons are not true symmetries of our curvature flows.

This paper is laid out as follows: in \cref{sec:prelim} we define our notational conventions and recall some standard definitions and identities. In \cref{sec:basic_evolution} we recall some standard evolution equations and commutators. The next section, \cref{sec:main_evolution} contains the bulk of our calculations where we derive the evolution equations for the main quantities we study. Then, applying these calculations we derive the Harnack inequalities in \cref{sec:harnack}. In this section, we present several variants depending on the strength of our assumptions. To finish, we prove preservation of convexity in \cref{sec:convexity} to justify our assumption of convexity for the Harnack inequality.
...
&\hp{=} + ct\Big\{2n\fr{\p''\p}{\p'}\br{\Box\p + \p\p^{ij}(h^2)_{ij} - b^{ij}\nabla_i \p \nabla_j \p}+\br{\zeta'-n\fr{\p''\p}{\p'}}\p^{ij}(h^2)_{ij}\p\\
&\hp{=+t}+ \zeta'\operatorname{tr}(\dot{\p})\p  + 2\p^2\p'H+\br{n\br{2\fr{\p''}{\p'}-\fr{\p''^{2}\p}{\p'^{3}}+\fr{\p'''\p}{\p'^{2}}}-\zeta''}\p^{ij}\nabla_{i}\p\nabla_j\p \\
...
\eq{\label{WeakHarnackEv12}\del_t\chi_1-\Box\chi_1 &= \br{\frac{\b-\t}{\delta\varphi} + \varphi^{ij}(h^2)_{ij} + c\frac{\d-1}{\d} \operatorname{tr}(\dot{\p})}\chi_1+\fr{c\operatorname{tr}(\dot{\p})\p}{\d}(tc\operatorname{tr}(\dot{\p})+2\d) \\
&+ t\p^{ij,kl}\br{\eta_{ij}+c\p g_{ij}}\br{\eta_{kl}+c\p g_{kl}}+t\br{2b^{il}\p^{jk}-\fr{\p^{ij}\p^{kl}}{\d\p}}\eta_{ij}\eta_{kl} \\
&+ tc\left\{2\p^2\p^{ij}h_{ij}+\left[\br{\p^{ij}h_{ij}+\p}b^{ir}-2\p^{ir}\right]b^{j}_{r}\nabla_{i}\p\nabla_{j}\p\right\}.
}
\end{proposition}
\pf{
